...
public class Subscriber {
    ...
    private final static int MAX_BOOKS_CHECKED_OUT = 10;
        // maximum number of books to be checked out
    // one subscriber
    ...
    public void checkoutBook(Book book) {
        if (book.isAvailable()) // check if the book is available
            if (booksCheckedOut.size()
                < MAX_BOOKS_CHECKED_OUT) { // check if the subscriber did not already
                ...
                System.out.println(
                    "Checkout Failed: You cannot check out more than " + MAX_BOOKS_CHECKED_OUT
                        + "books.");
            }
