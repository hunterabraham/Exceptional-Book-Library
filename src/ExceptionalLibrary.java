...
public class ExceptionalLibrary {
  ...
  public ExceptionalLibrary(String address, String librarianUsername, String librarianPassword) {
    ...
    this.librarian =
      new Librarian(librarianUsername, librarianPassword); // creates the librarian
    // of this library
  ...
  /**
   * Returns the books ArrayList for the library. Used as a helper method in
   * ExceptionLibraryTests.java
   *
  ...
  public ArrayList<Book> findBookByAuthor(String author) {
    ArrayList<Book> foundBooks =
      new ArrayList<>();// create an empty ArrayList to store found books
  ...
  /**
   * Parses subscriber's pin code. If a NumberFormatExceptions is hit when parsing, the method
   * throws a ParseException.
   *
   * @param s           The string to be parsed
   * @param errorOffset The index of s in commands[]
   * @return String s as and int
   * @throws ParseException Exception if parsing an Integer doesn't work
   */
  public int parsePinCode(String s, int errorOffset) throws ParseException {
    int stringVal = 0;
    try {
      stringVal = Integer.parseInt(s);
    } catch (NumberFormatException e) {
      throw new ParseException("Pin parsing error.", errorOffset);
    }
    if (stringVal < 1000 || stringVal > 9999) {
      throw new ParseException("Pin checking error.", errorOffset);
    }
    return stringVal;
  }

  /**
   * Parses subscriber's bar code. If a NumberFormatException is hit when parsing, the method
   * throws a ParseException
   *
   * @param s           The string to be parsed
   * @param errorOffset The index of s in commands[]
   * @return String s as an int
   * @throws ParseException Exception if parsing an Integer doesn't work
   */
  public int parseCardBarCode(String s, int errorOffset) throws ParseException {
    ...
    try {
      ...
      throw new ParseException("Bar code parsing error.", errorOffset);
    }
    if (!Subscriber.checkCardBarCode(stringVal)) {
      throw new ParseException("Bar code checking error.", errorOffset);
    }
    return stringVal;
  }
  ...
  /**
   * Parses book ID. If a NumberFormatException is hit when parsing, the method throws a
   * ParseException
   *
   * @param s           The string to be parsed
   * @param errorOffset The index of s in commands[]
  ...
  public int parseBookId(String s, int errorOffset) throws ParseException {
    ...
    try {
      ...
      throw new ParseException("Book ID parsing error.", errorOffset);
    }
    return stringVal;
  }
    ...
    if (book != null) { // book found --> remove the book
      if (book.isAvailable()) // check if the book is available
        ...
        System.out.println("You cannot remove a non available book. This book has been "
          + "checked out by the subscriber nÂ° " + book.getBorrowerCardBarCode()
  ...
  public void parseRunLibrarianCheckoutBookCommand(String[] commands) throws ParseException {
    ...
    this.parseCardBarCode(commands[1], 1); // Try to parse card bar code
    this.parseBookId(commands[2], 2); // Try to parse Book ID
    Book book = this.findBook(Integer.parseInt(commands[2]));
    Subscriber subscriber = this.findSubscriber(Integer.parseInt(commands[1]));
    if (book != null
      && subscriber != null) { // book ID correct - book found && subscriber found
  ...
  public void parseRunLibrarianRemoveBookCommand(String[] commands) throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);
    this.parseBookId(commands[1], 1); // Try to parse book ID
  ...
  public void parseRunSubscriberReturnBookCommand(String[] commands, Subscriber subscriber)
    throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);
    this.parseBookId(commands[1], 1); // Try to parse bookID
  ...
  public void parseRunSubscriberFindBooksByTitleCommand(String[] commands, Subscriber subscriber)
    ...
    this.checkCommandArgumentsCount(commands, 2); // check that commands[] is the right length
    ArrayList<Book> bookList = this.findBookByTitle(commands[1]); // create books list of
    // books with matching title
    if (!bookList.isEmpty()) // Display books
      displayBooks(bookList);
    else
      System.out.println("No books match your search."); // If books is empty, print error
    // message
  ...
  public void parseRunLibrarianSaveBooksCommand(String[] commands) throws ParseException {
    this.checkCommandArgumentsCount(commands, 2);
    File file = new File(commands[1]); //Initialize file object
    PrintWriter writer = null;
    try {
      writer = new PrintWriter(file); //Create new PrinterWriter object around the
      // created file
      for (int i = 0; i < books.size(); i++) { //Iterate through dialog ArrayList
        writer.print(books.get(i).getTitle()); //Print ArrayList element row by row
        writer.print(":");
  ...
  /**
   * Helper method for parseRunLibrarianLoadsBooksCommand(). Takes the read file and adds its
   * elements to the books ArrayList.
   *
   * @param booksList The list of books with titles and authors from the file
   * @param fileName  The name of the file being loaded
   */
  public void splitBooksList(ArrayList<String> booksList, String fileName) {
    String[] tempArr;
    // iterate through list, if list's string has length 0, skip that line.
    for (int i = 0; i < booksList.size(); i++) {
      if (booksList.get(i).length() == 0) {
        continue;
      }
      try {
        tempArr = booksList.get(i).split(":");
        tempArr[0] = tempArr[0].trim();
        tempArr[1] = tempArr[1].trim();
        Book book = new Book(tempArr[0], tempArr[1]);
        books.add(book);
      } catch (ArrayIndexOutOfBoundsException | NullPointerException e) {
        System.out.println(
          "Error: Found incorrectly formatted line in file " + fileName + ": " + booksList
            .get(i));
      }
    }
  }
  ...
  public void parseRunLibrarianLoadBooksCommand(String[] commands) throws ParseException {
    ...
    Scanner scnr = null; //Create scanner object
    try { //Try to
      scnr = new Scanner(file); //Create scanner around file
      ArrayList<String> booksList = new ArrayList<>();
      while (scnr.hasNextLine()) {
        booksList.add(scnr.nextLine());
      }
      splitBooksList(booksList, commands[1]);
    } catch (FileNotFoundException e) { //If there is an IOException, catch it and
   ...
   *                        is invalid or contains a syntax error
   */
  public void parseRunLoginAsSubscriber(String[] commands, Scanner scanner)
    throws ParseException {
    this.checkCommandArgumentsCount(commands, 3);
    Subscriber subscriber = this.findSubscriber(Integer.parseInt(commands[1]));
    if (subscriber != null) {
